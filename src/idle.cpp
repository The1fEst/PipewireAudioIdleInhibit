...
void Idle::block_systemd()
{
    ...
    int ret = sd_bus_call_method(bus, "org.freedesktop.login1", "/org/freedesktop/login1",
                                 "org.freedesktop.login1.Manager", "Inhibit", &error, &message, "ssss", "idle",
                                 "pipewire-audio-idle-inhibit", "Audio is playing", "block");
    if (ret < 0)
    {
...
void wayland_registry_handler(void *data, struct wl_registry *registry, uint32_t id, const char *interface,
                              uint32_t version)
{
    ...
    {
        self->compositor =
            static_cast<struct wl_compositor *>(wl_registry_bind(registry, id, &wl_compositor_interface, 1));
    }
    else if (strcmp(interface, "zwp_idle_inhibit_manager_v1") == 0)
    {
        self->inhibit_manager = wl_registry_bind(registry, id, &zwp_idle_inhibit_manager_v1_interface, 1);
    }
}
...
void Idle::block_wayland()
{
    ...
    struct zwp_idle_inhibit_manager_v1 *manager = static_cast<struct zwp_idle_inhibit_manager_v1 *>(inhibit_manager);
...
void Idle::cleanup_wayland()
{
    ...
    if (inhibit_manager)
    {
        zwp_idle_inhibit_manager_v1_destroy(static_cast<struct zwp_idle_inhibit_manager_v1 *>(inhibit_manager));
