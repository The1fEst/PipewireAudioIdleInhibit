...
static bool is_ignored(const std::string &name, const std::set<std::string> &ignore_set)
{
    if (name.empty())
    {
        return false;
...
static void evaluate_streams(PwContext *ctx)
{
    ...
    for (const auto &pair : ctx->nodes)
    {
        ...
        if (node->is_pulse ? contains_dummy(node->media_name) : contains_dummy(node->target_name))
        {
            continue;
        }
...
static void registry_global(void *data, uint32_t id, uint32_t permissions, const char *type, uint32_t version,
                            const struct spa_dict *props)
{
    ...
    NodeType node_type = is_jack ? NodeType::Jack : is_input ? NodeType::Input : NodeType::Output;
    ...
    node->proxy = static_cast<struct pw_proxy *>(pw_registry_bind(ctx->registry, id, type, PW_VERSION_NODE, 0));
    if (!node->proxy)
    {
        delete node;
        return;
