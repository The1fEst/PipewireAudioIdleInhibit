...
Data::Data(SubscriptionType subscriptionType) : subscriptionType(subscriptionType), ignoreConfig(load_ignore_config())
{
...
void Data::printTable()
{
    ...
    std::cout << std::left << std::setw(max_len) << "App | Input | Output" << std::endl;
    std::cout << std::string(max_len, '-') << "-|-------|---------" << std::endl;
    for (const auto &pair : activeApps)
    {
        std::cout << std::left << std::setw(max_len) << pair.first << " | " << (pair.second.input ? "*    " : "     ")
...
void Data::printWayBar()
{
    ...
    std::cout << "{\"text\": \"\", \"alt\": \"" << text << "\", \"tooltip\": \"\", \"class\": \"" << text << "\"}"
              << std::endl;
}